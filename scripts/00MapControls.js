...
function CreateOneObjectOnSQUAREMap(OneObject, LineType) {
	...
	ImageFullPath = ImageFullPath + LineType.MapTokensPath(GetExpansionPath(LineType.AllData[recoverMonsterBaseName(OneObject.title)].expansion));
	var doorStatus = "";
...
function CreateOneObjectOnHEXMap(OneObject, LineType) {
	...
	ImageFullPath = ImageFullPath + LineType.MapTokensPath(GetExpansionPath(LineType.AllData[SingleID].expansion));
	var doorStatus = "";
	if (OpenedClosedType == "OC") {
		if (LineType.elementName == "door") {
			doorStatus = " closed";
